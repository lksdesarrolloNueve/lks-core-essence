...
@Component({
    selector: 'dev-creditos',
    moduleId: module.id,
    templateUrl: './dev-creditos.component.html',
    providers: [
        {
            provide: DateAdapter,
...
export class DevCreditosComponent implements OnInit {

    //Declaracion de variables
    formDevengamiento: UntypedFormGroup;
    listaDetalleCreditos = [];
    columnsDCreditos: string[] = ['cvePoliza', 'periodo', 'ejercicio',
   ...
   */
    ...
    listaDetalleDevengamientos(accion: any) {
        ...
        this.service.getListByID(path, 'listaDevengamientoCredito').subscribe(detalle => {
            this.blockUI.stop();
            this.listaDetalleCreditos = detalle;
            this.dataSourceDevengamientos = new MatTableDataSource(this.listaDetalleCreditos);
    ...
    aplicarDevengamientos() {
        ...
        const dialogRef = this.dialog.open(verificacionModalComponent, {
            data: {
                titulo: 'Generar Devengamiento Créditos',
        ...
        dialogRef.afterClosed().subscribe(result => {
            ...
            if (result === 0) {
                ...
                this.service.registrarBYParametro(path, 'devengamientoCreditos').subscribe(result => {
                    this.blockUI.stop();
                    if (result[0][0] === '0') {
    ...
    cancelarDevengamientos(devengamiento) {
        ...
        const dialogRef = this.dialog.open(verificacionModalComponent, {
            data: {
                titulo: 'Cancelar Devengamiento Créditos',
                body: '¿Esta seguro de realizar el proceso?'
            }
        });
        ...
        dialogRef.afterClosed().subscribe(result => {
            ...
            if (result === 0) {
                ...
                this.service.registrarBYParametro(path, 'devengamientoCreditos').subscribe(response => {
                    this.blockUI.stop();
